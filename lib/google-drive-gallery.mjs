...
function galleryImageUrl(fileId) {
  return `/.netlify/functions/gallery-image?id=${encodeURIComponent(fileId)}`;
}
...
export function buildGalleryPayload({ rootFolderId, projects = [], files = [] }) {
  ...
  for (const file of files) {
    if (!file?.id || !file.mimeType?.startsWith(IMAGE_MIME_PREFIX)) continue;
  ...
  const recentWorkPhotos = files
    .filter((file) => file?.id && file.mimeType?.startsWith(IMAGE_MIME_PREFIX) && file.parents?.includes(rootFolderId))
